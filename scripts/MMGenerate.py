...
def main(args=None, callback=ERmain):
    if not args:
        ...
        options = get_settings()
    logging.info(f"Entrance Randomizer Version: {__version__}")
                        ...
                        for yaml in weights_cache[path]:
                            if category_name is None:
                                for category in yaml:
                                    ...
                                    logging.info(f"Meta: {key} >> {option}")
                                    if key in Options.CommonOptions.type_hints:
...
def roll_meta_option(option_key, game: str, category_dict: Dict) -> Any:
    ...
    options = MMRWorld.options_dataclass.type_hints
...
def handle_option(ret: argparse.Namespace, game_weights: dict, option_key: str, option: type(Options.Option), plando_options: PlandoOptions):
    if option_key in game_weights:
        try:
            ...
            player_option.verify(MMRWorld, ret.name, plando_options)
    else:
...
def roll_settings(weights: dict, plando_options: PlandoOptions = PlandoOptions.bosses):
    ...
    for option_key, option in MMRWorld.options_dataclass.type_hints.items():
